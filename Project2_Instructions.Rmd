...
knitr::kable(user_lost, caption="Which user was filtered out?")

# Print the logs that are associated with this userid.
knitr::kable(data[c(data$userid == user_lost$userid), ])
```

> ###### *The userid HBA 2262 was removed because every attempt by this user was a FAILED attempt without a valid maxsessionuid. Therefore I am comfortable discounting this user from further analysis and report that there were 762 unique users.*
...
# Reorder the dataframe for readability and add a column for intervals.
data_log <- data_log%>% select(userid, type, maxsessionuid, start_result, end_result, start_time, end_time)
data_log$interval <- difftime(data_log$end_time, data_log$start_time, units = "mins")

data_log %>% group_by(type) %>% summarize(average = mean(interval))
```
